...
def get_args():
    ...
    parser.add_argument("--max-epoch", type=int, default=50001)
    parser.add_argument("--grad-step", type=int, default=50)
    parser.add_argument("--policy-recon-weight", type=float, default=0.01)
    parser.add_argument("--weight-decay", type=float, default=0)
    parser.add_argument("--clip-val", type=float, default=0.9)
    parser.add_argument("--device", type=str, default="cuda:0" if torch.cuda.is_available() else "cpu")

    parser.add_argument("--info", type=str, default="stochastic_recon_loss")
...
def main(args=get_args()):
    ...
    if args.load_dataset_path is None:
        ...
        dataset = SequenceDataset(dataset, traj_len=args.traj_len, device=args.device)
    else:
        dataset = {}
    ...
    log_dirs = make_log_dirs(
        task_name,
        args.algo_name, args.seed, vars(args), record_params=["deterministic", "grad_step", "embedding_dim", "policy_recon_weight", "info"]
    )
    ...
    dynamics.train_dynamics(dataset, args.batch_size, logger, max_epoch=args.max_epoch, eval_freq=1000)
