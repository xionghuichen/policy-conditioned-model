...
def get_args():
    ...
    parser.add_argument("--load-exp-path", type=str, default="log/halfcheetah-medium-replay-v2/pcm&deterministic=True&grad_step=50&embedding_dim=128&policy_recon_weight=0.01&info=stochastic_recon_loss/seed_0&timestamp_24-0620-145156")
    parser.add_argument("--load-eval-policy-path", type=str, default="log/halfcheetah-medium-replay-v2/mopo&penalty_coef=0.5&rollout_length=5/seed_0&timestamp_23-0318-062424/checkpoint")
...
def main(eval_args=get_args()):
    ...
    dynamics.load_state_dict(torch.load(os.path.join(eval_args.load_exp_path, "checkpoint/dynamics_10001.pth"), map_location=eval_args.device))
    dynamics.eval()
