...
class ControlSuite:
  ...
  def __init__(self, task_name='humanoid_run'):
    ...
    if task_name == 'fish_swim':
      ...
      self.obs_shape = (24,)
      self.action_dim = 5
      self._keys = ['joint_angles', 'target', 'upright', 'velocity']
      ...
      self.obs_shape = (67,)
      self.action_dim = 21
      self._keys = ['com_velocity', 'extremities', 'head_height', 'joint_angles', 'torso_vertical', 'velocity']
      ...
      self.obs_shape = (44,)
      self.action_dim = 5
      self._keys = ['arm_pos', 'arm_vel', 'hand_pos', 'object_pos', 'object_vel', 'target_pos', 'touch']
      ...
      self.obs_shape = (24,)
      self.action_dim = 6
      self._keys = ["height", "orientations", "velocity"] 
      ...
      self.obs_shape = (12,)
      self.action_dim = 2
      self._keys = ['dist_to_target', 'position', 'target_position', 'touch', 'velocity']
...
def create_dataset(root_path, task_name, shapes, uint8_features, num_threads):
  filenames = [os.path.join(root_path, task_name, 'train-00000-of-00001')]
  file_ds = tf.data.Dataset.from_tensor_slices(filenames)
  example_ds = file_ds.interleave(
      functools.partial(tf.data.TFRecordDataset, compression_type='GZIP'),
      cycle_length=tf.data.experimental.AUTOTUNE,
  ...
  example_ds_parse = example_ds.map(map_func, num_parallel_calls=num_threads)

  counter = 0
  for res in tfds.as_numpy(example_ds_parse):
