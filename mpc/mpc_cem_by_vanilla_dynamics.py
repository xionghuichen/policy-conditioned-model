...
LOAD_DYNAMICS_PATH = {
    "halfcheetah-medium-replay-v2": "log/halfcheetah-medium-replay-v2/vanilla-dynamics&deterministic=True&info=None/seed_0&timestamp_23-0409-080039"
}


def get_args():
    parser = argparse.ArgumentParser()
    parser.add_argument("--algo-name", type=str, default="vanilla-dynamics-mpc-cem")
...
class MPCAgent:
    ...
    def cost_function(self, current_obs, action_seq, num_particals):
        ...
        for t in range(seq_len):
            next_obss, rewards, _, _ = self.dynamics.step(obss, action_seq[:, t])
...
def main(eval_args=get_args()):
    ...
    info = np.load('../pam/pam_info.npy', allow_pickle=True)
    saved_infos = []
    for i in range(len(info)):
        print(i)
        obs = info[i]["obs"]
        action_seq = info[i]["action_seq"]
        costs = agent.cem_optimizer.cost_function(obs, action_seq, 1)
        saved_infos.append({'obs': obs, 'action_seq': action_seq, 'costs': -costs})
    np.save('vanilla_info.npy', saved_infos, allow_pickle=True)
